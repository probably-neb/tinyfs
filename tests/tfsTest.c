...
/* simple helper function to fill Buffer with as many inPhrase strings
         * as possible before reaching size */
int
...
/* This program will create 2 files (of sizes 200 and 1000) to be read from or
 * stored in the TinyFS file system. */
int run_tfstest() {
    char readBuffer;
    ...
    if ((err = fillBufferWithPhrase(phrase1, afileContent, afileSize)) < 0) {
        perror("failed");
        return err;
    }

    bfileContent = (char *)malloc(bfileSize * sizeof(char));
    if ((err = fillBufferWithPhrase(phrase2, bfileContent, bfileSize)) < 0) {
        perror("failed");
        return err;
    }
    ...
    /* now, was there already a file named "afile" that had some content? If we
    * can read from it, yes!
    *  * If we can't read from it, it presumably means the file was empty.
    *   * If the size is 0 (all new files are sized 0) then any "readByte()"
    * should fail, so
